...
const Panquecas = ({navigation}) => {
let [flatListItems, setFlatListItems] = useState([]);

  useEffect(() => {
    db.transaction((tx) => {
      tx.executeSql('SELECT * FROM tbl_recipe where r_id = 3',
      [],
      (tx, results) => {
        var temp = [];
  ...
  let listItemView = (item) => {
    return (
      <View
        key={item.r_id}
        style={styles.list}>
        <Image source={{uri: 'https://mesaesabor.com.br/wp-content/uploads/2021/07/56-Receita-de-Panqueca-Colorida-com-Molho-Branco-Vegana.png'}}
          style={styles.img}/>
